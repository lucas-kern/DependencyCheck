...
hostedSuppressionsForceUpdate  | Whether the hosted suppressions file will update regardless of the `autoupdate` setting. If both `autoupdate` and `hostedSuppressionsForceUpdate` are false then the hosted suppressions file will not be created or updated.                                            | false
hostedSuppressionsUrl          | The URL to a mirrored copy of the hosted suppressions file for internet-constrained environments. To download as a local copy of the file to be used while scanning.                                   | https://jeremylong.github.io/DependencyCheck/suppressions/publishedSuppressions.xml
hostedSuppressionsValidForHours| Sets the number of hours to wait before checking for new updates from the `hostedSuppressionsUrl`.                                                       | 2
